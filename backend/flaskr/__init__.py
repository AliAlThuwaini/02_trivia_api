...
def create_app(test_config=None):
  ...
  '''
    ...
    def delete_question(question_id):
        ...
        try:
            qus_to_delete = Question.query.filter(
                Question.id == question_id).one_or_none()
            # In case tried to delete a non-existant question, it should abort
            # with 404: resource not available
        ...
        # In case try block failed which means something went wrong, should
        # abort with 422 code: unprocessable
  ...
  '''
    ...
    def search_questions():
        ...
        try:
            # make sure search term doesn't return empty result or else
            # abort with resource not found
            ...
            if search_term:
                ...
                # This should extract the uniques current categories out of
                # the retrieved questions from searching activity
                ...
                return jsonify({
                    ...
                    # Note: although I returned current_categories,
                    # I did not observe a difference!!
  ...
  '''
    ...
    def quiz_question():
        ...
        # check, if user selected category, pop up related questions, else use
        # all collection
        if quiz_category:
        ...
        if len(q_id) != len(previous_questions):
            ...
            new_q_id = random.choice(
                [id for id in q_id if id not in previous_questions])
            previous_questions.append(new_q_id)
            curr_question = Question.query.filter(
                Question.id == new_q_id).one_or_none().format()
        else:
